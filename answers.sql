...
SELECT models.name, brands.name, brands.headquarters from brands JOIN models
ON brands.name = models.brand_name where models.name = 'Mustang';
...
SELECT COUNT (*) FROM models WHERE models.brand_name ='Ford';
...
SELECT models.name, models.brand_name AS brand_name, brands.founded FROM models 
...
WHERE b.discontinued IS NULL and b.name IS NOT NULL;
...
SELECT b.name,
  ...
  ORDER BY b.name, m.name collate "C.UTF-8", m.year;
...
FROM models AS m WHERE m.year = ANY
(SELECT brands.founded FROM brands WHERE brands.founded IS NOT NULL);